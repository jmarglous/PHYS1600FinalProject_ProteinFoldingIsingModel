...
class Ising2D (object):
    ...
    def stateLattice(self, L, chainlength, AApositions):
        ...
        Locates positions of hydrophobic residues on entire lattice
        '''
        state = .5*np.ones((self.L, self.L), int)
        for i in range(chainlength):
            state[int(AApositions[0, i, 0]), int(AApositions[0, i,1])] = AApositions[0, i,3]
        return state
    ...
    def basicFig(self, save = False):
        ...
        for i in range(self.L):
            ax1.scatter(np.arange(self.L), i*np.ones(self.L), c = "#03b1fc", s = 5)
        ...
        ax1.set_xlim([self.L/4, 3*self.L/4])
        ax1.set_ylim([self.L/4, 3*self.L/4])
        ...
        for i in range(self.L):
            ax2.scatter(np.arange(self.L), i*np.ones(self.L), c = "#03b1fc", s = 5)
        ...
        ax2.set_xlim([self.L/4, 3*self.L/4])
        ax2.set_ylim([self.L/4, 3*self.L/4])
